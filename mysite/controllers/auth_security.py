...
def auth_signup_post():
    ...
    username = request.form.get('username')
    password = request.form.get('password')
    mycursor.execute("SELECT * FROM user WHERE username=%s OR email=%s;", (username, email))
    user = mycursor.fetchone()
    if user:
    ...
    mycursor.execute("INSERT INTO user(email, username, password, role, est_actif) VALUES(%s, %s, %s, %s, %s);", (email, username, password, 'ROLE_client', 1))
